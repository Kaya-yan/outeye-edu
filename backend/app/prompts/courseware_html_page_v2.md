<!-- 提示词模板：HTML 课件逐页生成 courseware_html_page · 版本 v2 · 2026-09-06
     ③ 两阶段生成第二阶段：规划器已产出页面蓝图，本提示词按蓝图逐页生成内容区 HTML（一次一页）。
     精讲页五要素为硬契约：原文段落/段落主旨/长难句解剖(主干提取+成分说明)/语言点/衔接点评。
     少样本示例区嵌入金标准样例页（The Jeaning of America 第 1 段逐段精讲页，类名已按骨架契约适配）。
...
【任务目的】只生成本页的内容区 HTML。成功标准：内容全部取材于教案与课文原文（不编造）；精讲页五要素齐全；长难句解剖落在原文具体词句上；投影可读、焦点明确、交互克制而有效。
