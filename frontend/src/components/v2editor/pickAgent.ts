...
export function buildPickAgentScript(channel: string): string {
  ...
  return [
    ...
    "var css=(d.payload&&d.payload.css)||'';",
    ...
    "st.textContent=css;",
    "if(curSel){setTimeout(function(){if(curSel&&curSel.parentNode){var r=curSel.getBoundingClientRect();send('ve:rect',{rect:{x:r.left,y:r.top,w:r.width,h:r.height}});}},60);}",
    "}",
    "if(d.type==='ve:verify'){",
